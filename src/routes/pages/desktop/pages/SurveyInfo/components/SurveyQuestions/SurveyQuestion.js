...
const SurveyQuestion = ({
  ...
  const { questions_id, questions_nm, survey_replys, questions_type } =
    questionData;
  ...
  const passiveReplyList = survey_replys.map((answer, idx) => ({
    ...
    label: answer.replys_rate === 9999 ? '단답식 답변' : answer.replys_value,
    value: idx,
  }));
   ...
   */
  const replysCheck = () => {
    const [temp] = questionData.survey_replys.filter(
      (item) => item.replys_rate === 9999
    );
    return temp;
  };
  ...
  const activeReplyCardTemplate = survey_replys.map((answer, idx) => {
    const { replys_value, replys_id, replys_rate } = answer;
    ...
    const activeCardReplyTemplate =
      activeAnswer === replys_id ? (
        ...
        >
          <span className="active-question-passive-reply">
            {replys_rate === 9999 ? '단답식답변' : replys_value}
          </span>
        </div>
      );
      ...
      <div className="question-box">
        <Card
          ...
          extra={
            <>
              ...
              <Button
                className="question-add-button"
                disabled={replysCheck()}
                style={{
                  marginLeft: '10px',
                }}
                onClick={() => {
                  // alert(JSON.stringify(questionData));
                  onAddAnswer(questionData, true);
                }}
              >
                단답 선택지 추가
              </Button>
              ...
              >
                {questionData.questions_usage ? '질문숨김' : '질문숨김해제'}
              </Button>
        ...
        >
          <div className="question-answer-box">
            {questions_type === 'TEXT' ? '단답형' : activeReplyElementTemplate}
          </div>
        </Card>
      </div>
    </div>
