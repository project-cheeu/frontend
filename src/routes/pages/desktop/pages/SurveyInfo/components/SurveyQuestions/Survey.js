...
const Survey = ({ survey, refresh }) => {
  ...
  // 댓글 추가 처리
  const onAddReply = (questionData, text = false) => {
    const { questions_id, survey_replys, questions_type } = questionData;
    if (survey_replys.length >= 12) {
      MessageAlert.warning('답변은 12개까지 등록가능합니다.');
      return;
    }
    if (questions_type === 'TEXT') {
      MessageAlert.warning('단답 답변은 선택지 추가가 불가능합니다.');
      return;
    }
    let newReply = {};
    if (text) {
   ...
   */
  const onHide = (questionData) => {
    URSurveyApi.toggleQuestion(questionData).then(() => {
      refresh();
    });
  };
